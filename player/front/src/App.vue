...
const formatISOTime = (isoTimeStr : string) => {
  // 容错：如果时间字符串为空/无效，返回空字符串
  if (!isoTimeStr) return '';
  
  try {
    // 解析ISO时间字符串为Date对象
    const date = new Date(isoTimeStr);
    // 格式化为 "YYYY-MM-DD HH:mm:ss" 格式（按需调整）
...
const checkStatus = async () => {
  try {
    ...
    switch (status.code) {
      case 0: {      
        mode.value = 'checkin'; 
        deviceIP.value = status.ip
        deviceMac.value = status.mac
        break;
    }
      case 1:
      case 2: {
        mode.value = (status.code == 1) ? 'checkout' : 'checkover';
        stuName.value = status.stuName;
        stuNum.value = status.stuNum.toString();
        checkInTime.value = formatISOTime(status.checkInTime);
        checkOutTime.value = formatISOTime(status.checkOutTime);
        deviceIP.value = status.ip
        deviceMac.value = status.mac
        break
      };
